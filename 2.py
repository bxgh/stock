...
def kdayClose(inc):  #启动日线收盘任务
    # enter四个参数分别为：间隔事件、优先级（用于同时间到达的两个事件同时执行时定序）、被调用触发的函数，
    # 给该触发函数的参数（tuple形式）
    schedule.enter(inc, 0, execute_kdayClose, (inc,))
    schedule.run()
...
if __name__ == '__main__':
    ...
    schedKdayCloseTime0=datetime.datetime(year,month,day,16,10)    #设置日线收盘时间，当天16:30
    schedKdayCloseTime1=datetime.datetime(year,month,day+1,16,10)    #设置日线收盘时间，次日16:30

    if now>schedOpenTime0 :
      incOpen=(schedOpenTime1-now).seconds
    else :
      incOpen=(schedOpenTime0-now).seconds

    if now>schedKdayCloseTime0 :
      incKdayClose=(schedKdayCloseTime1-now).seconds
    else :

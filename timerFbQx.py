...
if __name__ == '__main__':
    ...
    getHisdata=ExcFunc.conf.get(timerType,'getHisdata') #是否下载历史数据
    ...
    openTimehh=int(openTime[0:2])        #设置开盘小时
    openTimemm=int(openTime[2:4])        #设置开盘分钟
    closeTimehh=int(closeTime[0:2])      #设置收盘小时
    closeTimemm=int(closeTime[2:4])      #设置收盘分钟
    ...
    if ExcFunc.baseFunc.isTradeDay==1 and getHisdata=='0' : #交易日当天数据下载，执行任务循环。
        ExcFunc.MarketOpen()                       
        ontimer(incOpen,incClose)   # incOpen：开盘初始化 incClose：收盘作业
        
    if getHisdata =='1': #获取历史数据，单任务

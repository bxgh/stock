  ...
  def calcResult(self):
    ...
    for h5qfqfile in os.listdir(self.kdayH5Qfq_dir): 
      ...
      if df.size>0:
        ...
        #如果股价是先跌后上涨，则重新计算此区间最高股价和对应日期,参照600011区间走势
        if highDateDiff>lowerDateDiff:  
          dfReCalcHigh=dfRes1[dfRes1['trade_date']>lowestDateRecalc]
          dfReCalcHigh=dfReCalcHigh.set_index(['trade_date'])
          try:
           highest=dfReCalcHigh['high'].max()          
           highDate=dfReCalcHigh[dfReCalcHigh['high']==highest].index.tolist()[0]
           highestDate=dt.strptime(highDate, "%Y%m%d")
           highDateDiff=(tradeDate-highestDate).days     
          except:    #该区间股价创新低，参照600687该区间走势
            highest=lowest
            highDate=tradeDate.strftime('%Y%m%d')
            highDateDiff=0
        ...
        df=df.tail(1)
        #计算区间出现最高价格后回落的最低价格以及最低价格出现的日期
        dfRes2 = dfRes1[dfRes1['trade_date']>highDate]
        if dfRes2.size>0:
            dfRes2=dfRes2.set_index(['trade_date'])
            lowest1=dfRes2['low'].min()
            lowestDate1=dfRes2[dfRes2['low']==lowest1].index.tolist()[0]
            lowestDate1=dt.strptime(lowestDate1, "%Y%m%d")
            lowerDateDiff1=(tradeDate-lowestDate1).days            
            df['lowest1']=lowest1
            df['lowestDate1']=lowestDate1 
            df['lowerDateDiff1']=lowerDateDiff1
        else:    
            df['lowest1']=-1
            df['lowestDate1']=tradeDate 
            df['lowerDateDiff1']=0
        dfResult.append(df)
        # print(dfResult)
